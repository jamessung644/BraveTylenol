...
def choose_generation_profile(
    ...
    if _EMERGENCY.search(user_text):
        return GenerationProfile(
            ...
            max_tokens=1280,
            prompt_suffix=(
                "This may be an emergency: put urgent action first, then give brief safe "
                "steps while help is coming. Aim for at most 180 words."
            ),
        )
    if len(user_text) >= 700 or len(messages) >= 5 or _COMPLEX.search(user_text):
        return GenerationProfile(
            ...
            max_tokens=1280,
            prompt_suffix=(
                "Synthesize the clinical context without restating it. Prioritize the "
                "decision-relevant details. Aim for at most 180 words."
            ),
        )
    return GenerationProfile(
        kind="routine",
        max_tokens=768,
        prompt_suffix="Aim for at most 120 words.",
    )
