...
rag다. 어느 모드에서도 동일한 의료법·건강 AI 안전 프롬프트가 먼저 적용되므로 passthrough가
진단·처방 경계를 우회하지 않는다.

## 5. 의료법·건강 AI 안전 경계

서비스의 intended use는 일반 건강정보 및 웰니스 지원이다. 개인의 질병을 확정·배제하거나
개별 약물의 시작·중단·변경·용량을 결정하지 않으며, 처방전·진단서·의뢰서를 생성하지 않는다.
응급 가능성이 있으면 119 또는 응급실 안내를 먼저 하고, 그 외에는 가능한 설명과 불확실성,
적절한 진료 수준·시점, 의료진에게 확인할 질문을 제공한다. 법률 질문은 공식 출처와 시행일을
확인하도록 검색 정책에 포함했다.

구체적인 법 조항 매핑과 출시 전 통제는 [의료법 안전 경계](docs/medical-act-safety.md) 및
[건강 AI 법률 제품 체크리스트](docs/health-ai-law-product-checklist.md)를 참고한다. 프롬프트는
위험을 줄이는 한 계층일 뿐이며, 실제 공개 전 한국 의료·개인정보·의료기기 전문 법률 검토가
필요하다.
