...
2. Generation의 `tool_decision` phase에만 strict `retrieve_relevant_content` 하나를 등록한다. `direct_final`, `post_retrieval_final`, `mcp_failure_final`, `emergency_final`, `clean_recovery_final`에는 tool을 등록하지 않는다.
...
| Generation tools | `tool_decision`: `retrieve_relevant_content` 하나, 다섯 final phase: 없음 |
...
각 Generation phase request는 다음 순서를 사용한다.
...
6. Release `Dashboard-v1`에서는 adapter가 `status/items/note`를 exact 검증하고 실제 observed `cite_uid`·source status·version으로 단조 하향한 뒤 `retrieval-evidence-v4`로 Generation에 전달한다. Claim/relation을 model output에 없는데 새로 만들지 않고, escaped note는 비권위 limitation context로만 전달한다. [30](30_RETRIEVAL_PROMPT.md)의 `rich-v2` schema는 현재 design-only/hard-off이며, 별도 complete system prompt·함수·validator artifact와 L2 trial gate가 모두 생긴 release에서만 사용할 수 있다.
...
1. [20](20_GENERATION_PROMPT.md)의 여섯 phase block, [30](30_RETRIEVAL_PROMPT.md)의 Dashboard-v1 Retrieval block, [10](10_MCP_CATALOG.md)의 routing fragment와 exact strict tool schema를 결정적으로 compile한다. 문서 전체나 design-only rich-v2 block을 prompt에 포함하지 않는다.
...
4. Normal의 내부 `tool_decision`에만 strict `retrieve_relevant_content` 하나를 등록한다. `direct_final`, `post_retrieval_final`, `mcp_failure_final`, `emergency_final`, `clean_recovery_final`은 각각 frozen inbound와 trusted phase context로 새 transcript를 만들고 application tool을 하나도 등록하지 않는다. `submit_final_answer` 같은 두 번째 application tool도 추가하지 않는다.
...
2. 여섯 Generation phase prompt와 Retrieval prompt 및 서로 배타적인 tool registry
...
- [ ] `tool_decision`의 sole tool call이 strict `retrieve_relevant_content(query)`이고 다섯 final phase의 tool set이 비어 있음
...
- [ ] Invalid final은 이전 draft 없이 fresh `clean_recovery_final`을 정확히 한 번만 실행하고, 반복 invalid는 sanitized 502
